...
def test_c0_runner_generates_paired_representatives(tmp_path, monkeypatch):
    ...
    assert all(
        call["spec"]["show_inset_endpoints"] is True
        for call in representative_calls
    )
    assert outputs["representative"]["zalesak"]["with_endpoints"][
        "pdf"
    ].endswith("zalesak_appendix_c0_representative_with_endpoints.pdf")
