#!/usr/bin/env python3
"""
Run Zalesak experiments with ArcFacet.getPolyIntersectArea logging enabled.
"""
...
from util.logging.arc_intersect_area_logger import (
    disable_logging,
    enable_logging,
    get_stats,
)
...
DEFAULT_RESOLUTIONS = [0.50, 1.00]
DEFAULT_WIGGLES = [0.0, 0.3]
DEFAULT_SEEDS = [0]
DEFAULT_NUM_CASES = 1
...
def _default_artifact_dir():
    ...
    return Path("results/static/debug/arcintersect") / timestamp


def main():
    parser = argparse.ArgumentParser(
        description="Run Zalesak experiments with ArcFacet.getPolyIntersectArea logging"
    )
    ...
    parser.add_argument(
        "--artifact-dir",
        type=str,
        help="artifact directory (default: results/static/debug/arcintersect/<timestamp>)",
    )
    ...
    log_path = artifact_dir / "arc_intersect_area_calls.jsonl"
    ...
    print("=" * 80)
    print("ZALESAK ARC INTERSECT AREA HARVEST RUN")
    print("=" * 80)
