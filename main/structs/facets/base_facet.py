...
class LinearFacet(Facet):
    ...
    def advected(self, velocity, t, h, mode="RK4"):
        ...
        if (
            ...
            return ArcFacet(
                shiftcirclecenter, shiftcircleradius, shiftpLeft, shiftpRight
            )
...
class ArcFacet(Facet):
    ...
    def __init__(self, center, radius, pLeft, pRight):
        ...
        self.is_major_arc = isMajorArc(
            self.pLeft, self.pRight, self.center, self.radius
        )
    ...
    def advected(self, velocity, t, h, mode="RK4"):
        ...
        if (
            ...
            ):  # TODO replaced 6/10 because of numerical instabilities with nearly collinear points: getArea([shiftpLeft, shiftmid, shiftpRight]) < 0:
                shiftcircleradius *= -1
    ...
    def getPolyIntersectArea(self, poly):  # needs to be fixed TODO
        ...
        for i in range(0, len(arcpoints), 2):
            area += getArcArea(
                arcpoints[i], arcpoints[i + 1], self.center, abs(self.radius)
            )
